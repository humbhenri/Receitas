...
public class GenerateRecipeUseCase : IGenerateRecipeUseCase
{
    ...
    private readonly IGenerateRecipeAI generator;
    ...
    {
        Validate(request);
        var response = await generator.Generate(request.Ingredients);
        return new ResponseGeneratedRecipeJson
        {
            Title = response.Title,
            Ingredients = response.Ingredients,
            CookingTime = (Communication.Enums.CookingTime) response.CookingTime,
            Instructions = [.. response.Instructions.Select(i => new Instruction
            {
                Step = i.Step,
                Text = i.Text
            })],
